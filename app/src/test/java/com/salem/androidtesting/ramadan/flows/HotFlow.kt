...
class HotFlow {
    ...
    fun `test shared flow with SharingStarted Lazily`() = runTest {
        ...
        @Test
        fun `collect with eager strategy`() = runTest( context = UnconfinedTestDispatcher() ) {
            ...
            // SharingStarted.Lazily
            // ✅ 🚀 The flow starts immediately, even if no one is listening.
            // ❌ It does not wait for a listener to start.
